...
module.exports = {
  ...
  getUserLogin: (req, res) => {
    res.render('auth/userLogin', { title: 'User login', error : req.flash('error'),passError : req.flash('passError') });
  },
  ...
  getUserSignup: (req, res) => {
    res.render('auth/userSignup', { title: 'User signup', error: req.session.isExists });
  },
  ...
  getAdminLogin: (req, res) => {
    res.render('auth/adminLogin', { title: 'Admin login', error : req.session.error   });
    req.session.error = '';
  },
  ...
  getForgotPassword: (req, res) => {
    res.render('auth/forgotPassword', { title: 'Forgot Password', error: req.flash('error') });
  ...
  getSignupOtp: (req, res) => {
    ...
    res.render('auth/signupOtp', { title: 'OTP verification', warning: req.flash('error') })

  },

  //registered email giving page.
  getOtpEmail: (req, res) => {
    res.render('auth/otpEmail', { title: 'OTP email', warning : req.flash('error')});
  ...
  doSignupOtp: async (req, res) => {
    try {
      ...
      if (userEnteredOtp === req.session.otp) {
        const { email, password, lastname, firstname, phone } = req.session.userData;
        //hashing password
...
//
  doNewPassword: async (req, res) => {
    try {
      ...
      await userModel.collection.updateOne({ email: email }, { $set: { password: hashedPassword } });
      res.redirect('/login');
    } catch (error) {
      console.log(error);
    }
